(ns hfdl
  (:require [hyperfiddle.rcf :refer [tests]]
            [hfdl.lang :as h :refer [defnode vars main]]
...
(defmacro ! [f & args] `(unquote (m/ap (m/? (~f ~@args)))))
(defmacro test-dag [& body] "helper function for REPL only, wraps main, return sampler")

(tests
  "hello world"
  ; both clj and cljs
  (defnode printer [] (clojure.core/print "hello-world"))
  (def sampler (test-dag (printer)))
  @sampler
  ; hello-world
  := nil)
...
(tests
  ...
  (defnode foo [a b] (+ a b))
  (def !x (atom 2))
  (def sampler (test-dag (foo 1 ~(m/watch !x))))
  @sampler := 3
  (swap! !x inc)
  @sampler := 4)
...
(tests
  ...
  (def sampler (test-dag (render-shirt-size ~(m/watch !x))))
...
(tests
  ...
  (def !x (atom :mens-large))
  (def sampler (test-dag (render-shirt-size2 ~(m/watch !x))))
  @sampler := [:select {:selected :mens-large} [:option :mens-large]]
  (reset! !x :mens-medium)
  @sampler := [:select {:selected :mens-medium} [:option :mens-medium]])
...
(tests
  ...
  (def sampler (! hello-world ~(m/watch !needle)))
  @sampler
  := [:form
...
(tests
  ...
  (def !needle (atom "alice"))
  (def sampler (test-dag ~@(page-submission-detail ~@~(m/watch !needle))))
  @sampler
