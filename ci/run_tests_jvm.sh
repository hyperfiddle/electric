...
clojure -X:test \
    :dirs "[\"src\" \"test\"]" \
    :patterns "[\"^(?!hyperfiddle.(api|popover|txn|electric-fulcro|electric-httpkit|electric-jetty|spool|spec|impl.jetty|electric.impl.sunng|electric.impl.jetty9)|contrib.(datomic|test.datomic)).*\"]"
