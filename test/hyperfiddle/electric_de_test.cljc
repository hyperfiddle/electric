...
(tests "reactive closures - call them with (new)"
  (with ((l/single {} (tap (let [x 1, F (e/fn [] x)] [(number? x) (new F)]))) tap tap)
    % := [true 1]))
...
(tests "reactive fn"
  (with ((l/single {} (tap (new (e/fn [x] (inc x)) 1))) tap tap)
    % := 2))
...
(tests "lexical closure"
  (with ((l/single {} (tap (new (let [a 1] (e/fn [] a))))) tap tap)
    % := 1))
...
(tests "if with unwinding binding"
  (def !a (atom true))
  (with ((l/single {} (tap (new (binding [foo4 2] (e/fn [] (if (e/watch !a) foo4 (- foo4))))))) tap tap)
    % := 1
    (swap! !a not)
    % := -1))
...
(comment
  (def !xs (atom [1 2 3]))
  (def it ((l/single {} (tap (e/for-by identity [x (e/watch !xs)] (prn x) (inc x)))) tap tap))
  (swap! !xs conj 4)
  (it)
...
;;   (def !a (atom 0))
;;   (def !b (atom 0))
;;   (with ((l/single {} (tap @(doto !b (reset! (tap (new (m/watch !a))))))) tap tap)
;;     % := 0
;;     % := 0
;;     (swap! !a inc)
...
;;   (l/def x (e/watch !x))
;;   (l/def y (e/watch !y))
;;   (with ((l/single {} (tap (new (if (odd? x)
;;                               (e/fn [x] (* y x))
;;                               (e/fn [x] (* y x)))
;;                          x))) tap tap)
;;     % := 10
