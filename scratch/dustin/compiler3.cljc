...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Don't trace back passive nodes ;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
(defn gen-trace-pairs [prefixf analyzed-ast passives]
  (->> (map-indexed vector analyzed-ast)
       (remove (comp passives first))
       (mapv (fn [[idx _]] `{[~idx] (m/?? ~(prefixf idx))}))))
...
(defn gen-trace [prefixf analyzed-ast passives]
  `(trace! ~(prefixf 'tracef)
           (m/stream! (m/relieve merge (m/ap (amb= ~@(gen-trace-pairs prefixf analyzed-ast passives)))))))
...
(defn emit [{:keys [analyzed-ast prefix passives]
  ...
  (let [prefixf  (partial prefixer prefix)
    ...
    `(fn ~(mapv prefixf ['replayer 'tracef])
       (let [~@bindings]
         ~(gen-trace prefixf analyzed-ast passives)))))
