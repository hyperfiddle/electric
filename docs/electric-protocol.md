...
* multiplexing the successive diffs for these transfer sessions, according to the state of each expression
...
with reliable delivery and ordering. The link must support flow control in the writing direction, i.e. expose a
...
In the current implementation, websockets are used as the link implementation. Alternative implementations could be
considered in the future.

TODO: is link reconnection logic part of this document ?
...
other message MUST be sent as soon as the socket is ready to write.
...
each side of the link - an output on the local peer, an input on the remote peer. The output is a subscription to
...
* the *local request* count : incremented (resp. decremented) when a local event activates (resp. deactivates) a
channel that depends on this channel.
* the *remote request* count : incremented (resp. decremented) when an incoming message activates (resp. deactivates)
a channel that depends on this channel.
* the *pending toggle* count : incremented when the local request count is modified, decremented when the message
containing the change propagation is acknowledged.

Notation : PRI
* P: pending toggle flag - positive pending toggle count
* R: remote request flag - positive remote request count
* I: inferred request flag - positive local request count XOR pending toggle odd parity

Informally, the inferred flag tells if the local request was positive before the oldest unacknowledged message was
sent, i.e. the state known to the remote peer when its messages are processed.
...
## Session inhibition
A session that is in output state 101 must not sample its signal.

TODO : explain why

Example : an output is activated locally, then acked, then deactivated locally, not yet acked.
local request : 0
remote request : 0
pending toggle : 1

## Session invalidation
A session that is in input state 100 must ignore incoming diffs.
